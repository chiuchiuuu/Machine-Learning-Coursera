...
>  For instance, if we are trying to build a spam classifier for email, then $x^{(i)}$ may be some features of a piece of email, and $y$ may be 1 if it is a piece of spam mail, and 0 otherwise. Hence, $y\in \lbrace 0,1 \rbrace$. 0 is also called the **negative class**, and 1 the **positive class**, and they are sometimes also denoted by the symbols “$-$” and “$+$.” Given $x^{(i)}$, the corresponding $y^{(i)}$ is also called the label for the training example.
...
so
$$
\begin{align*}& \theta^T x \geq 0 \implies y = 1 \newline& \theta^T x < 0 \implies y = 0 \newline\end{align*}
$$
...
$$
\min_\theta\ \dfrac{1}{2m}\  \left[  \sum_{i=1}^m \text{Cost}(h_{\theta}(x^{(i)}), y^{(i)}) + \lambda\ \sum_{j=1}^n \theta_j^2 \right ]
$$

- $\lambda$ is the **regularization parameter**. It determines how much the costs of our theta parameters are inflated. Too large may cause underfitting
- $j$ start from 1 because we don't want to penalize $\theta_0$
...
|                   Regularized GD Algorithm                   |
| :----------------------------------------------------------: |
| $\begin{align} & \text{Repeat} \lbrace \\ & \ \ \ \ \theta_0 := \theta_0 - \alpha\ \frac{1}{m}\ \sum_{i=1}^m (h_\theta(x^{(i)}) - y^{(i)})x_0^{(i)} \\ & \ \ \ \ \theta_j := \theta_j(1 - \alpha\frac{\lambda}{m}) - \alpha\frac{1}{m}\sum_{i=1}^m(h_\theta(x^{(i)}) - y^{(i)})x_j^{(i)} \\ & \rbrace \end{align}$ |
...
\newline& \text{where}\ \ L = \begin{bmatrix} 0 & & & & \newline & 1 & & & \newline & & 1 & & \newline & & & \ddots & \newline & & & & 1 \newline\end{bmatrix} \in \mathbb R^{n+1\times n+1}
\end{align*}
$$
Recall that if $m \lt n$, then $X^TX$ is non-invertible. However, when we add the term $\lambda L$, then $X^TX + \lambda \cdot L$ becomes invertible.
...
| :----------------------------------------------------------: |
| $\begin{align}& \text{Repeat} \lbrace \\ & \ \ \ \ \theta_0 := \theta_0 - \alpha \frac{1}{m} \sum_{i = 1}^m (h_\theta(x^{(i)}) - y^{(i)})x_0^{(i)} \\ &\ \ \ \ \theta_j := \theta_j - \alpha\frac{1}{m} \sum_{i = 1}^m( h_\theta(x^{(i)}) - y^{(i)})x_j^{(i)} , \quad j = 1, 2, 3, \cdots,n \\ &\rbrace \end{align}$ |
